...
def infer(g_body, g_face, smplx_model, rendertool, config, args):
    ...
    wav_name = os.path.split(cur_wav_file)[1]
    # please set save folder here. e.g. save_root = '/path/to/outputs/audio2FLAME'
    save_root = ''
    save_root = os.path.join(save_root, wav_name)
    os.mkdir(save_root)
    os.system(f'cp {cur_wav_file} {os.path.join(save_root, wav_name)}')
    np.save(os.path.join(save_root, wav_name.replace('wav', 'npy')), result_list[0].detach().cpu().numpy())
